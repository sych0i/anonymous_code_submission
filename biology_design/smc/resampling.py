...
def systematic_resample(log_weights: torch.Tensor):
    N = log_weights.shape[0]
    weights = normalize_weights(log_weights)
    cdf = torch.cumsum(weights, dim=0)
    # Guard searchsorted against floating-point cumsum values just below one.
    cdf[-1] = 1.0
...
def systematic_resample_population(
    particles: torch.Tensor,
    log_weights: torch.Tensor,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Fully resample N output slots within each independent SMC batch.

    Args:
        particles: Tensor shaped ``(B, N, ...)``.
        log_weights: Tensor shaped ``(B, N)``.

    Returns:
        A tuple ``(resampled_particles, ancestor_indices)``.  Resampling never
        crosses batch boundaries, and each batch retains exactly N output slots.
    """
    if particles.ndim < 2:
        raise ValueError(
            f"particles must have shape (B, N, ...), got {tuple(particles.shape)}"
        )
    if log_weights.ndim != 2:
        raise ValueError(
            f"log_weights must have shape (B, N), got {tuple(log_weights.shape)}"
        )
    if particles.shape[:2] != log_weights.shape:
        raise ValueError(
            "particles and log_weights must share their (B, N) dimensions; "
            f"got {tuple(particles.shape[:2])} and {tuple(log_weights.shape)}"
        )

    ancestor_indices = torch.stack(
        [
            systematic_resample(log_weights[batch_index])
            for batch_index in range(log_weights.shape[0])
        ],
        dim=0,
    )
    gather_shape = (
    ...
    gather_indices = ancestor_indices.reshape(gather_shape).expand_as(particles)
    return torch.gather(particles, dim=1, index=gather_indices), ancestor_indices
