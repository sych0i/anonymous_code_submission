...
from transformers.utils import auto_docstring, get_torch_version, logging # type: ignore
    ...
    def forward(
        ...
        if input_ids is not None:
            if input_ids.is_floating_point():
                input_shape = input_ids.size()[:-1] # ignore the one-hot dimension
            else:
        ...
        if inputs_embeds is None:
            ...
            if input_ids.is_floating_point(): # type: ignore
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            if input_ids.is_floating_point():
                self.warn_if_padding_and_no_attention_mask(input_ids.argmax(dim=-1), attention_mask)
                input_shape = input_ids.size()[:-1] # ignore the one-hot dimension
            else:
