...
class MDLM(GenerativeModel):
    def __init__(self, model_name, config, tokenizer, num_steps=128, seq_len=389, device='cuda'):
        ...
        if checkpoint.get("format") != "protein-fitness-inference-v1":
            raise RuntimeError("expected the supplied inference-only checkpoint format")
        # Training loops, optimizer moments, callbacks, EMA copies, and absolute paths were
        # removed from the reviewer checkpoint; none are consulted by SMC-base inference.
        config.training.ema = 0.0
        self.model = MDLMDiffusion(config=config, tokenizer=tokenizer)
        incompatible = self.model.load_state_dict(checkpoint["state_dict"], strict=True)
        if incompatible.missing_keys or incompatible.unexpected_keys:
            raise RuntimeError(f"incompatible inference checkpoint: {incompatible}")
    ...
    def get_start(self, batch_size):
        if self.diffusion == "absorbing_state":
            return self.model.mask_index * torch.ones(
                (batch_size, self.length), dtype=torch.int64, device=self.device)
        if self.diffusion == "uniform":
            # The terminal distribution of uniform corruption is categorical-uniform over the
            # complete model vocabulary.  Starting UDLM from the absorbing model's all-mask
            # prior changes p_T and invalidates the initial SMC correction/path target.
            return torch.randint(
                0, self.S, (batch_size, self.length), dtype=torch.int64, device=self.device)
        raise NotImplementedError(
            f"Start distribution for diffusion type {self.diffusion} is not implemented.")

    @torch.no_grad()
    def p_sample(self, x, t, t_next=None, hard=True, guidance=None, log_p_x0=None):
        t = t/self.timestep
        # [BEFORE] t_next = t/self.timestep  # bug: used already-normalized t, collapsing every DAPS ODE step to t≈0
        if t_next is not None:
            t_next = t_next/self.timestep  # DAPS: explicit t_next → normalize properly for multi-step ODE
        else:
            t_next = t/self.timestep  # CLS guidance, NOS, uncond: None → single-step denoising (t≈0)
        ...
        # else: caller supplies the cached base log p(x0|xt). SMC-grad reuses it to construct
        # p(x_s|x_t), then applies its categorical value-gradient tilt to that transition.
    ...
    @torch.no_grad()
    def compute_log_p_x0(self, x, t):
        '''
        log p(x0 | x_t) at integer timestep t -- the same backbone call `p_sample` makes
        internally when it isn't handed a `log_p_x0` override. Exposed so callers that need the
        same quantity for more than one purpose at the same (x, t) (e.g. SMC's value-rollout and
        its subsequent reverse-transition step) can compute it once and pass it into `p_sample`
        via `log_p_x0=`, instead of paying for the forward pass twice.
        '''
        t_norm = torch.as_tensor(t, dtype=torch.float32, device=x.device) / self.timestep
        sigma_t, _ = self.model.noise(t_norm)
        sigma_t = sigma_t * torch.ones(x.shape[0]).to(x.device)
        if sigma_t.ndim > 1:
            sigma_t = sigma_t.squeeze(-1)
        return self.model.forward(x, sigma_t)

    def parameterize_raw_logits(self, raw_logits, hard_xt, sigma=None):
        '''Apply this model's clean-token parameterization to raw backbone logits.

        ``hard_xt`` is deliberately separate from the tensor that produced
        ``raw_logits``.  A differentiable caller can feed a soft one-hot relaxation
        through the backbone while SUBS still uses the corresponding discrete state
        for its mask and deterministic-copy rules.  The parameterization helpers are
        functional and do not mutate ``raw_logits``.
        '''
        if raw_logits.ndim != 3:
            raise ValueError(
                'raw_logits must have shape (B, L, V), got '
                f'{tuple(raw_logits.shape)}')
        if hard_xt.ndim != 2 or tuple(hard_xt.shape) != tuple(raw_logits.shape[:2]):
            raise ValueError(
                'hard_xt must have shape matching raw_logits[:2], got '
                f'hard_xt={tuple(hard_xt.shape)} and '
                f'raw_logits={tuple(raw_logits.shape)}')
        hard_xt = hard_xt.to(device=raw_logits.device, dtype=torch.long)

        if self.parameterization == 'subs':
            return self.model._subs_parameterization(
                logits=raw_logits, xt=hard_xt)
        if self.parameterization == 'd3pm':
            return self.model._d3pm_parameterization(logits=raw_logits)
        if self.parameterization == 'sedd':
            if sigma is None:
                raise ValueError('sigma is required for SEDD parameterization')
            sigma = self.model._process_sigma(sigma)
            return self.model._sedd_parameterization(
                logits=raw_logits, xt=hard_xt, sigma=sigma)
        return raw_logits

    def compute_log_p_x0_soft(self, x_probs, hard_xt, t,
                              return_raw_logits=False):
        '''Differentiable clean-token prediction from a soft current state.

        Args:
            x_probs: Float tensor ``(B, L, V)``.  It may be an exact one-hot or
                a probability relaxation and is embedded by a weighted sum of the
                generation backbone's vocabulary embeddings.
            hard_xt: Integer tensor ``(B, L)`` corresponding to the discrete
                current state.  SUBS uses it to preserve revealed tokens exactly.
            t: Integer diffusion timestep in the wrapper's ``0, ..., T`` scale.
            return_raw_logits: Return the backbone logits before SUBS/D3PM/SEDD
                parameterization when true.

        Unlike ``compute_log_p_x0``, this method intentionally has no
        ``torch.no_grad`` decorator: gradients flow from its output through every
        transformer block to ``x_probs``.  Existing integer sampling continues to
        use the no-grad method above.
        '''
        if x_probs.ndim != 3:
            raise ValueError(
                'x_probs must have shape (B, L, V), got '
                f'{tuple(x_probs.shape)}')
        if not x_probs.is_floating_point():
            raise TypeError('x_probs must be a floating-point tensor')
        if x_probs.shape[-1] != self.S:
            raise ValueError(
                f'x_probs final dimension must equal vocab size {self.S}, got '
                f'{x_probs.shape[-1]}')
        if hard_xt.ndim != 2 or tuple(hard_xt.shape) != tuple(x_probs.shape[:2]):
            raise ValueError(
                'hard_xt must have shape matching x_probs[:2], got '
                f'hard_xt={tuple(hard_xt.shape)} and x_probs={tuple(x_probs.shape)}')

        t_norm = torch.as_tensor(
            t, dtype=torch.float32, device=x_probs.device) / self.timestep
        sigma_t, _ = self.model.noise(t_norm)
        sigma_t = sigma_t * torch.ones(
            x_probs.shape[0], device=x_probs.device)
        if sigma_t.ndim > 1:
            sigma_t = sigma_t.squeeze(-1)

        raw_logits = self.model.forward(
            x_probs, sigma_t, return_raw_logits=True)
        if return_raw_logits:
            return raw_logits
        return self.parameterize_raw_logits(
            raw_logits, hard_xt=hard_xt, sigma=sigma_t)
    ...
    def _nos_denoise(
      ...
      t: torch.Tensor,
        ...
        t = t/self.timestep
        t_next = t/self.timestep # parameterization is in [0,1] # NOTE: _nos_denoise receives no t_next arg — single-step denoising is intentional here
        sigma_t, _ = self.model.noise(t)
        sigma_s, _ = self.model.noise(t_next)
