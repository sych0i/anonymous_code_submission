...
class MDLMDiffusion(L.LightningModule):
    def __init__(
        ...
        # self.eval_model_tokenizer = AutoTokenizer.\
    ...
    def _subs_parameterization(self, logits, xt):
        if xt.ndim != 2:
            raise ValueError(
                'SUBS parameterization requires hard token indices with shape '
                f'(B, L), got {tuple(xt.shape)}. For a soft backbone input, '
                'request raw logits and parameterize them with the corresponding '
                'hard state.')

        # Apply the mask exclusion functionally.  The old in-place update mutated
        # the caller's raw logits, which made it unsafe to reuse them for a
        # differentiable soft-input score and a hard transition kernel.
        mask_adjustment = F.one_hot(
            torch.as_tensor(self.mask_index, device=logits.device),
            num_classes=logits.shape[-1]).to(logits.dtype)
        logits = logits + self.neg_infinity * mask_adjustment
        ...
        # For unmasked tokens return a deterministic copy distribution.  Build
        # it without writing into ``logits`` so autograd retains an unmodified
        # raw-logit branch for masked positions.
        unmasked_indices = (xt != self.mask_index)
        copy_logits = torch.full_like(logits, self.neg_infinity)
        copy_logits = torch.scatter(
            copy_logits,
            -1,
            xt.long().unsqueeze(-1),
            torch.zeros_like(copy_logits[..., :1]))
        return torch.where(unmasked_indices.unsqueeze(-1), copy_logits, logits)
    ...
    def forward(self, x, sigma, x_emb=None, cond=None,
        ...
        """Return parameterized log scores, or raw backbone logits on request.

        ``return_raw_logits=True`` is the differentiable escape hatch for a
        soft ``(B, L, V)`` backbone input.  In particular, SUBS needs the
        corresponding hard state to impose its deterministic-copy constraint;
        callers can therefore obtain raw logits here and apply
        ``_subs_parameterization(raw_logits, hard_xt)`` explicitly.
        """
