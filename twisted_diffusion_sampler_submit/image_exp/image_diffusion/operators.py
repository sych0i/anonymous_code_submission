...
class ConditioningMethod(ABC):
    ...
    def classifier_prob(self, xt, x0_hat, measurement, classifier, guidance_scale=1.0, return_grad=True, 
        ...
        # guidance_scale powers the selected likelihood.  Scaling logits
        # *before* softmax changes the x-dependent normalization and is not
        # p(y|x)^gamma, which breaks the reward R=exp(r/alpha) used by both
        # TDS and VISTA.  Compute log p once, then multiply it by gamma.
        y = measurement # label 
        ...
        if return_grad:
            with th.enable_grad():
                ...
                selected = guidance_scale * log_probs_original[range(len(logits)), y]
            ...
            if return_full_prob_original_scale:
                return selected, log_probs_original
            return selected 
